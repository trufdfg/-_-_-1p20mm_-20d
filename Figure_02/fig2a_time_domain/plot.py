...
for kf,g in df.groupby('kf_mgkg'):
    ax.plot(g['time_ps'],g['amplitude_au'], style[int(kf)], lw=1.2, color=color[int(kf)], label=f'{int(kf)} mg kg$^{{-1}}$')
ax.set_xlabel('Time (ps)'); ax.set_ylabel('Amplitude (a.u.)')
ax.set_ylim(-1100,2050); ax.set_xlim(-5,120); ax.set_yticks([-1000,0,1000,2000])
clean_axes(ax); ax.text(-0.18,1.05,'a', transform=ax.transAxes, fontsize=11, fontweight='bold', va='top')
savefig(fig,'fig2a_time_domain')
