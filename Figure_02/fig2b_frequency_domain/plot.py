...
for kf,g in df.groupby('kf_mgkg'):
    ax.plot(g['frequency_thz'],g['amplitude_db'], style[int(kf)], lw=1.1, color=color[int(kf)], label=f'{int(kf)}')
ax.set_xlabel('Frequency (THz)'); ax.set_ylabel('Amplitude (dB)')
ax.set_xlim(0,3); ax.set_ylim(-205,10); ax.set_yticks([-200,-150,-100,-50,0])
clean_axes(ax); ax.text(-0.18,1.05,'b', transform=ax.transAxes, fontsize=11, fontweight='bold', va='top')
savefig(fig,'fig2b_frequency_domain')
