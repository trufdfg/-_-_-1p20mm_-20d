...
ax.barh(df['candidate_band'], df['composite_score'], color=COLORS['blue'])
ax.set_xlabel('Composite score'); ax.set_ylabel('Candidate band')
ax.set_xlim(0,0.9)
clean_axes(ax, grid_axis='x'); ax.text(-0.12,1.05,'a', transform=ax.transAxes, fontsize=11, fontweight='bold', va='top')
savefig(fig,'fig3a_pca_band_screening')
