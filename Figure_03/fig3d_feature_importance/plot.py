...
df=pd.read_csv(BASE/'data.csv').iloc[::-1]
fig, ax=plt.subplots(figsize=(3.3,3.0), dpi=300)
ax.barh(df['feature'], df['importance'], color=COLORS['green'])
ax.set_xlabel('XGBoost feature importance')
ax.set_xlim(0,0.27)
clean_axes(ax, grid_axis='x'); ax.text(-0.12,1.05,'d', transform=ax.transAxes, fontsize=11, fontweight='bold', va='top')
savefig(fig,'fig3d_feature_importance')
